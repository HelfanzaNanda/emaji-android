...
interface CycleContract{
    fun getCycles(token : String, listener : ArrayResponse<Cycle>)
}

class CycleRepository (private val api : ApiService) : CycleContract{
    override fun getCycles(token: String, listener: ArrayResponse<Cycle>) {
        api.getCycles(token).enqueue(object : Callback<WrappedListResponse<Cycle>>{

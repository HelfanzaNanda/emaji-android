...
class HomeFragment : Fragment(R.layout.fragment_home), HomeListener {
    ...
    override fun click(tool: Tool) {
        homeViewModel.setNameTool(tool.name!!)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == 100 && resultCode == Activity.RESULT_OK && data != null){
            val scanResult = data.getStringExtra("CODE")
            scanResult?.let {
                homeViewModel.validatedTool(Constants.getToken(requireActivity()), it, homeViewModel.getNameTool().value!!)
            }
        }
    }
